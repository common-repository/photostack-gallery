...
$(function() {	
	...
	*/
	$ps_albums.children('div').bind('click',function(){
		...
		//alert();
		$ps_container.css('top',offset.top);
		var album_name 	= 'album' + parseInt($elem.index() + 1);
		var postid = $(this).attr('id');
		var $loading 	= $('<div />',{className:'loading'});
		$elem.append($loading);
		$ps_container.find('img').remove();
		//$.get(ajax_url, {album_name:album_name} , function(data)  // need to change this for wordpress ajax
		$.post(MyAjax.ajaxurl,{action : 'myajax-submit', album_name:album_name, postid: postid}, function( data )     
